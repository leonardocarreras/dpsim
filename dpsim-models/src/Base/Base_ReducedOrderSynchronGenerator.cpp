...
void Base::ReducedOrderSynchronGenerator<VarType>::addExciter(
  ...
  if (!mExciter) {
    SPDLOG_LOGGER_ERROR(this->mSLog,
                        "addExciter called with null exciter on {}", *this->mName);
    return;
  }
  if (!params) {
    SPDLOG_LOGGER_WARN(this->mSLog,
                       "addExciter called without parameters on {}", *this->mName);
    return;
  }
    ...
    Real Ta, Real Ka, Real Te, Real Ke, Real Tf, Real Kf, Real Tr)
{
    ...
    mExciter  = std::make_shared<CPS::Signal::ExciterDC1Simp>(name, this->mLogLevel);
    mExciter->setParameters(params);
    mHasExciter = true;

    SPDLOG_LOGGER_INFO(this->mSLog,
        "Attached ExciterDC1Simp to {} (Ka={:.3g}, Ta={:.3g}, Kef={:.3g}, Tef={:.3g}, "
