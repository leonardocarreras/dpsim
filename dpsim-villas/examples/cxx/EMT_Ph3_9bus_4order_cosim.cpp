...
const std::string buildShmemFollowerConfig() {
    ...
    DPsim::Logger::get("ShmemFollowerCosim3PhNineBus")
            ->debug("Config for Node:\n{}", config);
    return config;
}
...
SystemTopology buildTopology(CommandLineArgs &args,
  ...
  seqNumForRTDS->addTask(CPS::UpdateTaskKind::UPDATE_ON_GET,
    ...
    const bool blockOnImport = !useFollowerShmem;
    const bool syncOnImport = !useFollowerShmem;
    intf->addImport(inFromRTDS_SeqRTDSAttribute, blockOnImport, syncOnImport);
    intf->addImport(inFromRTDS_SeqDPsimAttribute, blockOnImport, syncOnImport);
    intf->addImport(cs->mCurrentRef->deriveCoeff<Real>(0, 0), blockOnImport,
                                    syncOnImport);
    intf->addImport(cs->mCurrentRef->deriveCoeff<Real>(1, 0), blockOnImport,
                                    syncOnImport);
    intf->addImport(cs->mCurrentRef->deriveCoeff<Real>(2, 0), blockOnImport,
                                    syncOnImport);

    intf->addExport(outToRTDS_SeqDPsimAttribute);
    intf->addExport(inFromRTDS_SeqRTDSAttribute);
    intf->addExport(n6EMT->mVoltage->deriveCoeff<Real>(0, 0));
    intf->addExport(n6EMT->mVoltage->deriveCoeff<Real>(1, 0));
    intf->addExport(n6EMT->mVoltage->deriveCoeff<Real>(2, 0));
    if (!useFollowerShmem) {
        intf->addExport(seqNumForRTDS);
    }
...
int main(int argc, char *argv[]) {
    CommandLineArgs args(argc, argv, "cosim-9bus-4order", 0.00005, 1 * 60,
  ...
  bool log = args.options.find("log") != args.options.end() &&
    ...
    std::string transport = "fpga";
    if (args.options.find("transport") != args.options.end()) {
        transport = args.getOptionString("transport");
    }
    const bool useFollowerShmem = (transport == "shmem");

    std::string selectedConfig =
            useFollowerShmem ? buildShmemFollowerConfig() : buildFpgaConfig(args);
    std::string intfName = useFollowerShmem ? "ShmemInterface" : "FpgaInterface";

    auto villasIntf = std::make_shared<InterfaceVillasQueueless>(
            selectedConfig, intfName, spdlog::level::off);

    std::shared_ptr<InterfaceCosimSyncShmem> syncIntf = nullptr;
    ...
    if (useFollowerShmem) {
        ...
        syncIntf = std::make_shared<InterfaceCosimSyncShmem>(
        ...
        startAt = InterfaceCosimSyncShmem::toTimePoint(cfg.start_time_ns);
  ...
  if (log) {
    ...
    auto sys = buildTopology(args, villasIntf, useFollowerShmem, logger);
