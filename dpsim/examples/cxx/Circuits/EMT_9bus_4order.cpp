...
SystemTopology buildTopology(CommandLineArgs &args,
  ...
  if (logger) {
// Logging
logger->logAttribute("v1", n1EMT->attribute("v"));
logger->logAttribute("v2", n2EMT->attribute("v"));
logger->logAttribute("v3", n3EMT->attribute("v"));
logger->logAttribute("v4", n4EMT->attribute("v"));
logger->logAttribute("v5", n5EMT->attribute("v"));
logger->logAttribute("v6", n6EMT->attribute("v"));
logger->logAttribute("v7", n7EMT->attribute("v"));
logger->logAttribute("v8", n8EMT->attribute("v"));
logger->logAttribute("v9", n9EMT->attribute("v"));
...
int main(int argc, char *argv[]) {
  ...
  if (log) {
    logger =
        DataLogger::make(logFilename); //, args.duration, args.timeStep);
  }
